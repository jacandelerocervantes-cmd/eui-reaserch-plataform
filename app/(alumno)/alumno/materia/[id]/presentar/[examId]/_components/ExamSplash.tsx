...
export function ExamSplash({ examTitle, questionCount, durationMinutes, onStart, courseId, examId }: {
  examTitle: string;
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[70vh] text-center gap-8">
      ...
      {/* Piloto ai-tutor-sandbox, modo "exam_prep": SOLO aquí, antes de
          iniciar. El backend bloquea este modo en cuanto abre la ventana
          real del examen (exams.start_at) — ver verifyAlumnoSandbox en
          supabase/functions/_shared/auth.ts. Repaso de temario, nunca
          reactivos del examen real. */}
      <AiTutorChat courseId={courseId} contextType="exam_prep" examId={examId} title="Repasar antes del examen" />
    </div>
  );
}
