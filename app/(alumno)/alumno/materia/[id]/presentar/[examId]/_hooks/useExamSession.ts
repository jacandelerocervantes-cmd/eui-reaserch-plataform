...
export function useExamSession({ result, examId }: { result: FetchResult; examId: string }) {
  ...
  // ── Timing por pregunta (prerrequisito para detección de anomalías tipo
  // Isolation Forest, ver conversación de "estado del arte de herramientas
  // de integridad académica") — un timestamp por CADA vez que cambia la
  // respuesta de una pregunta (no solo la primera), para poder calcular
  // después features como "tiempo entre respuestas" y "# de cambios de
  // respuesta", que hoy no existen. No se usa en el bloqueo de 2/3
  // incidencias actual (eso sigue igual) — solo se guarda para análisis
  ...
  const handleAnswer = (questionId: string, value: AnswerValue) => {
    ...
    const existing = answerTimestampsRef.current[questionId] ?? [];
    answerTimestampsRef.current = { ...answerTimestampsRef.current, [questionId]: [...existing, new Date().toISOString()] };
  };
  ...
  const buildMetadata = () => {
    ...
    return {
      ...
      // answer_timing: un array de timestamps ISO por pregunta, uno por cada
      // vez que el alumno cambió esa respuesta (no solo la última). Todavía
      // no lo consume nada — es la materia prima para el Isolation Forest
      // (tiempo entre respuestas, # de cambios) cuando se construya esa
      // pieza. Guardarlo desde ya evita perder el historial de exámenes que
      // se presenten mientras tanto.
      answer_timing: answerTimestampsRef.current,
    };
  };
