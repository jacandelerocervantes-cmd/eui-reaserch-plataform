...
import { 
  Sparkles, Mail, CheckSquare, 
  Video, Plus, Loader2, AlertCircle,
  Zap, X, Save
} from "lucide-react";
...
export default function CentroDeMando() {
  ...
  type MailItem = { color: string; categoria: string; urgencia: number; remitente: string; titulo: string };
  type AgendaItem = { color: string; time: string; categoria: string; titulo: string };
  type TaskItem = { color?: string; titulo: string };
  const [dashboardData, setDashboardData] = useState({ emails: [] as MailItem[], agenda: [] as AgendaItem[], tasks: [] as TaskItem[] });
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setDashboardData({ emails: [], agenda: [], tasks: [] });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    } catch { alert("Error al conectar con Google Tasks"); }
  ...
  const handleCreateMeet = async (e: React.FormEvent) => {
    ...
    } catch { alert("Error al conectar con Google Calendar"); }
    finally { setIsSubmitting(false); }
  };
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <ExpandingButton
            ...
            durationMs={400}
            shadow="hover"
            colors={{ border: "#e2e8f0", hoverBg: "white", hoverText: "#1B396A" }}
          />
          <ExpandingButton
            icon={Video}
            label="Sesión Meet"
            ...
            durationMs={400}
            shadow="hover"
            colors={{ hoverBg: "#1B396A", hoverText: "white" }}
          />
        </div>
      </header>
