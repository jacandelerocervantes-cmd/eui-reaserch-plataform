...
import { CheckSquare, Plus, Star, Calendar, Trash2, ListChecks, Filter } from "lucide-react";
...
export default function ListaPendientes() {
  ...
  const taskLists = [
    { name: "Prioritarios", count: 3, color: "#ef4444" },
    { name: "Docencia", count: 8, color: "#3b82f6" },
    { name: "Investigación", count: 5, color: "#f59e0b" },
    { name: "Personales", count: 12, color: "#10b981" }
  ];
    ...
    <div style={{ padding: "40px", maxWidth: "1200px", margin: "0 auto", display: "grid", gridTemplateColumns: "280px 1fr", gap: "32px" }}>
      ...
      <aside style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{ backgroundColor: "white", borderRadius: "20px", border: "1px solid #e2e8f0", padding: "10px" }}>
          {taskLists.map(list => (
            <div key={list.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderRadius: "10px", cursor: "pointer", transition: "all 0.2s" }} onMouseEnter={e => e.currentTarget.style.backgroundColor = "#f8fafc"}>
              ...
              <span style={{ fontSize: "0.8rem", fontWeight: "800", color: "#94a3b8" }}>{list.count}</span>
            </div>
          ))}
        </div>
      </aside>
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h1 style={{ color: "#1B396A", fontSize: "2rem", fontWeight: "900", margin: 0 }}>Pendientes: Docencia</h1>
        ...
        <div style={{ backgroundColor: "white", borderRadius: "24px", border: "1px solid #e2e8f0", padding: "20px", display: "flex", flexDirection: "column", gap: "12px" }}>
          {[
            { t: "Subir actas de la Unidad 2 - Redes", d: "Mañana", p: "Alta" },
            { t: "Enviar retroalimentación a María R.", d: "Hoy", p: "Media" },
            { t: "Preparar material para laboratorio de viernes", d: "27 Feb", p: "Baja" }
          ].map((task, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px", backgroundColor: "#f8fafc", borderRadius: "16px", border: "1px solid #f1f5f9" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                <div style={{ width: "24px", height: "24px", borderRadius: "6px", border: "2px solid #cbd5e1", cursor: "pointer" }} />
                <div>
                  <div style={{ fontWeight: "700", color: "#1e293b", fontSize: "0.95rem" }}>{task.t}</div>
