...
function ConfigInvestigacionContent({ reloadKey, onRetry }: { reloadKey: number; onRetry: () => void }) {
  const hasMounted = useRef(false);
  const [loading, setLoading] = useState(true);
  const [result, setResult] = useState<FetchResult>({ ok: false, error: "" });
  const userId = result.ok ? result.userId : null;
  const [perfil, setPerfil] = useState<Profile>(DEFAULT_PROFILE);
  const [isSaving, setIsSaving] = useState(false);
  const [saveError, setSaveError] = useState<string | null>(null);
  const [lastSaved, setLastSaved] = useState<string | null>(null);

  const [showZoteroKey, setShowZoteroKey] = useState(false);
  const [orcidStatus, setOrcidStatus] = useState<"idle" | "checking" | "valid" | "invalid">("idle");
  const [zoteroStatus, setZoteroStatus] = useState<"idle" | "checking" | "valid" | "invalid">("idle");

  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchPerfil(reloadKey).then((r) => {
      if (!isMounted) return;
      setResult(r);
      if (r.ok) {
        setPerfil(r.perfil);
        setOrcidStatus(r.orcidValid ? "valid" : "idle");
        setZoteroStatus(r.zoteroValid ? "valid" : "idle");
      }
      setLoading(false);
    });
...
export default function ConfigInvestigacion() {
  ...
  return (
    <ConfigInvestigacionContent reloadKey={reloadKey} onRetry={() => setReloadKey((k) => k + 1)} />
  );
}
