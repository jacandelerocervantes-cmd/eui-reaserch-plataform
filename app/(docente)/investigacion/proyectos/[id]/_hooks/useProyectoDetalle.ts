...
export function useProyectoDetalleContent(proyectoId: string, reloadKey: number, onReload: () => void) {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [result, setResult] = useState<FetchResult>({ kind: "error", message: "" });
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchProyecto(proyectoId, router, reloadKey).then((r) => {
      if (!isMounted) return;
      setResult(r);
      setColaboradores(r.kind === "ok" ? r.colaboradores : []);
      setLoading(false);
    });
    return () => { isMounted = false; };
  }, [proyectoId, router, reloadKey]);
