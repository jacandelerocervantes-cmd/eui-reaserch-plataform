...
// Espejo de SubmissionFeatures/ValidatorResult en
// supabase/functions/_shared/logisticValidator.ts
type ValidatorFeatures = { suspiciousPasteCount: number; contributionPercent: number | null; totalRevisions: number };
type ValidatorResult = { probability: number; flagged: boolean; reasons: string[]; features: ValidatorFeatures };
...
function AuditoriaContent({
  courseId, assignmentId, submissionId, reloadKey, onRetry,
}: { courseId: string; assignmentId: string; submissionId: string; reloadKey: number; onRetry: () => void }) {
  ...
  const [orderedSubs, setOrderedSubs] = useState<{ id: string; status: string }[]>([]);
  const [isAnalyzingIntegrity, setIsAnalyzingIntegrity] = useState(false);
  const [integrityResult, setIntegrityResult] = useState<IntegrityResult | null>(null);
  // Validador independiente (Regresión Logística, no llama a Gemini) — corre
  // automáticamente después de "Analizar" porque consume las mismas señales
  // que esa llamada ya calculó (contribution_percent/integrity_flags).
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchAuditoria(courseId, assignmentId, submissionId, reloadKey).then((r) => {
      if (!isMounted) return;
      setResult(r);
      if (r.ok) {
        setScores(r.initialScores);
        setFeedback(r.initialFeedback);
        setIsPublished(r.isPublished);
        setOrderedSubs(r.orderedSubs);
      }
      // Estos se reinician al cambiar de alumno/recargar, igual que antes
      // ocurría al remontar el componente con `key`.
  ...
  }, [courseId, assignmentId, submissionId, reloadKey]);

  if (loading) return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><Loader2 className="animate-spin" size={48} color="#1B396A" /></div>
  );
  ...
  const handleAnalyzeIntegrity = async () => {
    ...
    try {
      ...
      // Validador independiente: no llama a Gemini, corre sobre las mismas
      // señales que acaba de calcular la línea de arriba (contribución,
      // pegados masivos) — reenviamos totalRevisions porque esa función no
      // lo persiste en `submissions`.
      const { data: valData, error: valError } = await supabase.functions.invoke('validate-submission-integrity', {
        body: { submission_id: submissionId, total_revisions: data.totalRevisions },
      });
      if (!valError && valData?.success) setValidatorResult(valData.data);
    } catch (e) {
  ...
  const handleIntegrityFeedback = async (verdict: 'confirmed_cheating' | 'false_positive') => {
    if (!validatorResult) return;
    try {
      const { error } = await supabase.functions.invoke('submit-integrity-feedback', {
        body: {
          submission_id: submissionId,
          predicted_probability: validatorResult.probability,
          features: validatorResult.features,
          verdict,
        },
      });
      if (error) throw error;
      setFeedbackSubmitted(true);
      setToast(verdict === 'confirmed_cheating' ? '✅ Registrado como confirmado.' : '✅ Registrado como falsa alarma.');
    } catch (e) {
      setToast(`❌ ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: "450px", borderLeft: "1px solid #e2e8f0", display: "flex", flexDirection: "column", backgroundColor: "#fdfdfd" }}>
          <div style={{ flex: 1, overflowY: "auto", padding: "35px" }}>
            ...
            {/* Validador independiente (Regresión Logística) — NO es Gemini,
                segunda opinión real. Nunca bloquea nada, solo puntúa. */}
            {validatorResult && (
              <div style={{ backgroundColor: "white", border: `1px solid ${validatorResult.flagged ? "#fecaca" : "#e2e8f0"}`, borderRadius: "16px", padding: "18px", marginBottom: "30px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", fontWeight: "800", color: "#1B396A", fontSize: "0.9rem", marginBottom: "10px" }}>
                  <ShieldAlert size={16} color={validatorResult.flagged ? "#dc2626" : "#94a3b8"} /> Validador Independiente
                </div>
                <div style={{ fontSize: "0.8rem", color: "#94a3b8", marginBottom: "10px" }}>
                  No usa IA generativa — es una regresión logística sobre las señales de arriba. Score: <b style={{ color: "#1B396A" }}>{validatorResult.probability.toFixed(2)}</b>
                </div>
                {validatorResult.reasons.length > 0 ? (
                  <ul style={{ margin: "0 0 12px", paddingLeft: "18px", fontSize: "0.82rem", color: "#7c2d12" }}>
                    {validatorResult.reasons.map((r, i) => <li key={i}>{r}</li>)}
                  </ul>
                ) : (
                  <div style={{ fontSize: "0.8rem", color: "#94a3b8", marginBottom: "12px" }}>Sin señales relevantes.</div>
                )}

                {feedbackSubmitted ? (
                  <div style={{ fontSize: "0.8rem", color: "#16a34a", fontWeight: 700 }}>Gracias, tu veredicto quedó registrado.</div>
                ) : (
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button
                      onClick={() => handleIntegrityFeedback('confirmed_cheating')}
                      style={{ flex: 1, border: "1px solid #fecaca", background: "#fef2f2", color: "#991b1b", borderRadius: "10px", padding: "8px", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer" }}
                    >
                      Sí era trampa
                    </button>
                    <button
                      onClick={() => handleIntegrityFeedback('false_positive')}
                      style={{ flex: 1, border: "1px solid #e2e8f0", background: "white", color: "#64748b", borderRadius: "10px", padding: "8px", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer" }}
                    >
                      Falsa alarma
                    </button>
                  </div>
...
export default function AuditoriaIndividualPage() {
  ...
  return (
    <AuditoriaContent courseId={courseId} assignmentId={assignmentId} submissionId={submissionId} reloadKey={reloadKey} onRetry={() => setReloadKey((k) => k + 1)} />
  );
}
