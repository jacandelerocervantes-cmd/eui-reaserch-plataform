...
async function fetchActividad(courseId: string, assignmentId: string): Promise<FetchResult> {
  try {
    // 1. Obtener Unidades
...
// Antes esta pantalla leía los datos con `use(resource)` (Suspense) — se
// cambió a un `useEffect` clásico (mismo patrón que useNuevaActividad.ts,
// ya verificado) porque con `use()` NINGÚN evento de clic funcionaba en toda
// la pantalla (selectores de modalidad, candado, criterios, "Regenerar con
// IA", "Cancelar" — confirmado con logging directo: los onClick nunca se
// ejecutaban pese a que el evento nativo sí llegaba al DOM). Escribir texto
// sí funcionaba, solo los clics quedaban rotos — un efecto colateral de
// React con `use()`/Suspense en este árbol que no vale la pena perseguir
// más a fondo cuando ya existe un patrón alterno probado en el propio código.
export function useEditarActividad(courseId: string, assignmentId: string) {
  const router = useRouter();
  const [reloadKey, setReloadKey] = useState(0);
  const [loading, setLoading] = useState(true);
  const [result, setResult] = useState<FetchResult>({ ok: false });
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchActividad(courseId, assignmentId).then((r) => {
      if (!isMounted) return;
      setResult(r);
      if (r.ok) {
        setFormData(r.formData);
        setRubrics(r.rubrics);
        setRequireAttendance(r.requireAttendance);
        setSelectedSessionId(r.selectedSessionId);
      }
      setLoading(false);
    });
