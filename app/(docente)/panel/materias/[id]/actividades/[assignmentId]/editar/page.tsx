...
import {
  Save, X, Wand2, Lock, Plus, ShieldCheck, AlertCircle, Loader2, Copy,
  User, Users, FileUp, Cloud, FileText, Table, Presentation
} from "lucide-react";
import ExpandingButton from "@/components/ui/ExpandingButton";
import DateTimeFieldMX from "@/components/ui/DateTimeFieldMX";
import DuplicateActivityModal from "./_components/DuplicateActivityModal";
    ...
    <div style={{ padding: "40px", width: "100%", flex: 1, maxWidth: "1200px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", paddingBottom: "20px", borderBottom: "1px solid #e2e8f0" }}>
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <ExpandingButton icon={Copy} label="Duplicar" onClick={() => setShowDuplicateModal(true)} variant="secondary" disabled={isSaving} size={42} radius={10} gap={10} padding="0 16px" fontWeight={600} fontSize="0.9rem" durationMs={300} />
      ...
      <form onSubmit={handleSave} style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: "30px", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div style={{ backgroundColor: "white", padding: "32px", borderRadius: "24px", border: "1px solid #e2e8f0", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.02)" }}>
            ...
            <DateTimeFieldMX disabled={isDeadlinePassed} value={formData.soft_deadline} onChange={(v) => setFormData({...formData, soft_deadline: v})} />
          </div>
...
export default function EditarActividadPage() {
  ...
  return <EditarActividadContent courseId={courseId} assignmentId={assignmentId} />;
}
