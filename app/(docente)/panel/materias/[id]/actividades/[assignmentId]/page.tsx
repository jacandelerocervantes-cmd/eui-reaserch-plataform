...
function DashboardEntregasContent({
  courseId, assignmentId, reloadKey, onReload,
}: { courseId: string; assignmentId: string; reloadKey: number; onReload: () => void }) {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchEntregas(courseId, assignmentId, reloadKey).then((r) => {
      if (!isMounted) return;
      setResult(r);
      setLoading(false);
    });
...
export default function DashboardEntregas() {
  ...
  return (
    <DashboardEntregasContent courseId={courseId} assignmentId={assignmentId} reloadKey={reloadKey} onReload={() => setReloadKey((k) => k + 1)} />
  );
}
