...
function ActividadesContent({ courseId, reloadKey, onRetry }: { courseId: string; reloadKey: number; onRetry: () => void }) {
  const router = useRouter();
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchActividades(courseId, reloadKey).then((r) => {
      if (!isMounted) return;
      setResult(r);
      setLoading(false);
    });
...
export default function ActividadesPage() {
  ...
  return (
    <ActividadesContent courseId={courseId} reloadKey={reloadKey} onRetry={handleRetry} />
  );
}
