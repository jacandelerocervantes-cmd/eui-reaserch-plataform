...
export function useAsistencia(courseId: string) {
  ...
  const guardarGeocercaModal = async () => {
    ...
    } catch { setFeedback({ type: "error", message: "Error al guardar GPS." }); } finally { setIsSaving(false); }
  };
  ...
  const startRadar = async () => {
    if (!geocerca) { setFeedback({ type: "error", message: "Configura el GPS primero." }); return; }
    setIsSaving(true);
    try {
      const res = await fetch("/api/attendance/session", {
        method: "POST",
    ...
    } catch { setFeedback({ type: "error", message: "Error al iniciar radar." }); } finally { setIsSaving(false); }
  };
  ...
  const guardarFinal = async () => {
    ...
    } catch (e) { setFeedback({ type: "error", message: e instanceof Error ? e.message : String(e) }); } finally { setIsSaving(false); }
  };
