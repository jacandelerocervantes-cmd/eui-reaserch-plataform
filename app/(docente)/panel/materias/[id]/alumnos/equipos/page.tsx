...
import { useEquiposContent } from "./_hooks/useEquipos";

function EquiposContent({ reloadKey, onReload }: { reloadKey: number; onReload: () => void }) {
  const courseId = useParams().id as string;
  const eq = useEquiposContent(courseId, reloadKey, onReload);

  if (eq.loading) return (
    <div style={{ padding: "60px", textAlign: "center" }}><Loader2 className="animate-spin" size={32} color="#1B396A" style={{ margin: "0 auto" }} /></div>
  );
...
export default function EquiposPage() {
  ...
  return (
    <EquiposContent reloadKey={reloadKey} onReload={() => setReloadKey((k) => k + 1)} />
  );
}
