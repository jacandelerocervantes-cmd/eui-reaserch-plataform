...
function ListaAlumnosContent({ courseId, reloadKey, onReload }: { courseId: string; reloadKey: number; onReload: () => void }) {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchStudents(courseId, reloadKey).then((r) => {
      if (!isMounted) return;
      setResult(r);
      setLoading(false);
    });
  ...
  // de admin (ver también enroll-manual/index.ts, modo "lookup").
  const [matriculaLookup, setMatriculaLookup] = useState<'idle' | 'checking' | 'found' | 'new'>('idle');

  useEffect(() => {
    const m = newStudent.matricula.trim();
    if (!showModal || m.length < 2) { setMatriculaLookup('idle'); return; }
    setMatriculaLookup('checking');
    const t = setTimeout(async () => {
      try {
        const { data } = await supabase.functions.invoke('enroll-manual', {
          method: 'POST',
          body: { mode: 'lookup', matricula: m },
        });
        if (data?.found && data?.profile) {
          setNewStudent((prev) => ({
            ...prev,
            apellido_paterno: data.profile.apellido_paterno || "",
            apellido_materno: data.profile.apellido_materno || "",
            nombres: data.profile.nombres || "",
            correo: data.profile.correo || "",
          }));
          setMatriculaLookup('found');
        } else {
          setMatriculaLookup('new');
        }
      } catch {
        setMatriculaLookup('new');
      }
  ...
  }, [newStudent.matricula, showModal]);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Quitar a este alumno de esta materia? No afecta su registro en otras materias.")) return;
    try {
      const { data, error } = await supabase.functions.invoke('enroll-manual', {
        method: 'POST',
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.headerActions}>
          ...
          <ExpandingButton icon={UserPlus} label="Agregar Alumno" onClick={() => setShowModal(true)} variant="primary" size={44} radius={10} gap={8} padding="0 12px" fontWeight={600} durationMs={300} iconSize={20} expandedLabelMaxWidth="150px" />
        </div>
      </div>
          ...
          <div className={styles.modalContent}>
            ...
            <form onSubmit={handleSaveStudent} className={styles.formGrid}>
              <div style={{ position: "relative" }}>
                ...
                {matriculaLookup === 'checking' && (
                  <Loader2 className="animate-spin" size={16} color="#94a3b8" style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)" }} />
                )}
              </div>

              {matriculaLookup === 'found' && (
                <div style={{ backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", color: "#1e40af", padding: "10px 14px", borderRadius: "10px", fontSize: "0.82rem", fontWeight: 600, display: "flex", alignItems: "center", gap: "8px" }}>
                  <CheckCircle2 size={16} style={{ flexShrink: 0 }} />
                  Esta matrícula ya está registrada — se usará su identidad existente. Solo un administrador puede corregir nombre/correo.
                </div>
              )}

              <input required disabled={matriculaLookup === 'found'} placeholder="Nombre(s)" className={styles.inputField} value={newStudent.nombres} onChange={(e) => setNewStudent({...newStudent, nombres: e.target.value})} />

              <div className={styles.formRow}>
                <input required disabled={matriculaLookup === 'found'} placeholder="Apellido Paterno" className={styles.inputField} value={newStudent.apellido_paterno} onChange={(e) => setNewStudent({...newStudent, apellido_paterno: e.target.value})} />
                <input disabled={matriculaLookup === 'found'} placeholder="Apellido Materno" className={styles.inputField} value={newStudent.apellido_materno} onChange={(e) => setNewStudent({...newStudent, apellido_materno: e.target.value})} />
              </div>
        ...
        <div className={styles.modalOverlay}>
          {importStep === 'upload' ? (
            <div className={styles.modalContent} style={{ textAlign: "center", position: "relative" }}>
              ...
              <h2 className={styles.modalTitle} style={{ marginBottom: "20px" }}>Importación Inteligente</h2>
              ...
              <div style={{ border: "2px dashed #cbd5e1", padding: "30px", borderRadius: "16px", cursor: "pointer", backgroundColor: selectedFile ? "#f0f7ff" : "transparent", transition: "all 0.2s" }}>
                ...
                <label htmlFor="file-ia" style={{ cursor: "pointer", display: "block" }}>
                  ...
                  <p style={{ fontSize: "0.78rem", color: "#94a3b8", marginTop: "4px" }}>
                    PDF, imagen, Excel o CSV — admite múltiples páginas y grupos
                  </p>
                </label>
              </div>
              ...
              <div style={{ marginTop: "24px", display: "flex", justifyContent: "center" }}>
                <ExpandingButton
                  ...
                  label={isAnalyzing ? "Analizando documento..." : "Analizar Documento"}
...
export default function ListaAlumnos() {
  ...
  return (
    <ListaAlumnosContent courseId={courseId} reloadKey={reloadKey} onReload={() => setReloadKey((k) => k + 1)} />
  );
}
