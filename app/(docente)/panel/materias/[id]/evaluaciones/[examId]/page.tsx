...
function RevisionContent({ courseId, examId, reloadKey, onReload }: { courseId: string; examId: string; reloadKey: number; onReload: () => void }) {
  ...
  const {
    ...
    handleNotify,
    handleSaveScore,
  } = useEvaluacionDetalleContent(courseId, examId, reloadKey, onReload);
    ...
    <div style={{ padding: "40px", maxWidth: "1300px", margin: "0 auto", backgroundColor: "#F8FAFC", minHeight: "100vh" }}>
      {feedbackToast && (
        <div style={{
          ...
          backgroundColor: feedbackToast.type === "success" ? "#dcfce7" : "#fee2e2",
          border: `1px solid ${feedbackToast.type === "success" ? "#bbf7d0" : "#fecaca"}`,
          color: feedbackToast.type === "success" ? "#166534" : "#991b1b",
            ...
            {filteredAlumnos.map((a) => (
              <tr key={a.id} style={{ borderBottom: "1px solid #f1f5f9", backgroundColor: a.entregado ? "white" : "#fafafa", transition: "0.2s" }}>
                ...
                <td style={{ padding: "22px 30px", color: "#1B396A", fontWeight: "800", fontSize: "1.1rem" }}>
                  {a.entregado ? <span style={{ backgroundColor: "#1B396A15", padding: "4px 10px", borderRadius: "8px", color: "#1B396A", fontWeight: "700" }}>{a.score_ia}</span> : "--"}
                </td>
                <td style={{ padding: "22px 30px" }}>
                  {a.entregado ? (
                    <input
      ...
      {/* MODAL DE CONFIRMACIÓN: Notificar Resultados por Correo */}
      {showNotifyConfirm && (
        <div
          style={{
          ...
          onClick={(e) => { if (e.target === e.currentTarget) setShowNotifyConfirm(false); }}
        >
          <div style={{ backgroundColor: "white", borderRadius: "20px", padding: "30px", maxWidth: "480px", width: "90%", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }}>
            ...
            <p style={{ color: "#334155", lineHeight: "1.6", margin: "0 0 24px 0", fontSize: "0.95rem" }}>
              Se enviará un correo electrónico con la calificación final a cada alumno con resultado publicado en esta evaluación. ¿Deseas continuar?
            </p>
...
export default function RevisionExamenPage() {
  ...
  const { reloadKey, onReload } = useEvaluacionDetalle(courseId, examId);

  return (
    <RevisionContent courseId={courseId} examId={examId} reloadKey={reloadKey} onReload={onReload} />
  );
}
