...
function AuditoriaContent({
  courseId, examId, studentId, reloadKey, onRetry,
}: { courseId: string; examId: string; studentId: string; reloadKey: number; onRetry: () => void }) {
  const v = useAuditoriaConsola(courseId, examId, studentId, reloadKey);
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", backgroundColor: "white" }}>
      {v.actionFeedback && (
        <div style={{
          ...
          backgroundColor: v.actionFeedback.type === "success" ? "#dcfce7" : "#fee2e2",
          border: `1px solid ${v.actionFeedback.type === "success" ? "#bbf7d0" : "#fecaca"}`,
          color: v.actionFeedback.type === "success" ? "#166534" : "#991b1b",
          padding: "14px 18px", borderRadius: "12px", fontWeight: "600", fontSize: "0.9rem",
          display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px",
          boxShadow: "0 10px 25px -5px rgba(0,0,0,0.15)",
        }}>
          <span>{v.actionFeedback.message}</span>
          <button
            type="button"
            onClick={() => v.setActionFeedback(null)}
...
export default function AuditoriaExamenIndividual() {
  ...
  const { reloadKey, retry } = useRevisionExamen(courseId, examId, studentId);

  return (
    <AuditoriaContent courseId={courseId} examId={examId} studentId={studentId} reloadKey={reloadKey} onRetry={retry} />
  );
}
