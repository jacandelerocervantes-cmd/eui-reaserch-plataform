...
export default function SimulacionExamenPage() {
  ...
  const renderAnswerArea = (q: SimQuestion, qIdx: number) => {
    ...
    if (q.type === 'matching') {
      const currentMatches = (s.answers[qIdx] ?? {}) as Record<number, number>;
      return (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {q.left?.map((l: string, li: number) => (
              <div key={li} style={{ padding: "16px 20px", borderRadius: "14px", border: "2px solid #f1f5f9", fontWeight: "700", color: "#1B396A", display: "flex", alignItems: "center" }}>{li + 1}. {l}</div>
            ))}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {q.left?.map((_: string, li: number) => (
              <select
                key={li}
                value={currentMatches[li] ?? ""}
                onChange={(e) => {
                  const value = e.target.value;
                  const next = { ...currentMatches };
                  if (value === "") delete next[li]; else next[li] = Number(value);
                  s.setAnswers({ ...s.answers, [qIdx]: next });
                }}
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: "#F8FAFC", padding: "60px 20px" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ textAlign: "left", backgroundColor: "white", padding: "40px", borderRadius: "32px", border: "1px solid #1B396A25", boxShadow: "0 20px 40px rgba(27, 57, 106, 0.05)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px", color: "#1B396A", marginBottom: "25px" }}>
              <Sparkles size={28} />
            ...
            {s.isAnalyzing ? (
              <div style={{ padding: "40px", textAlign: "center" }}>
                <Loader2 className="animate-spin" size={30} color="#1B396A" />
                <p style={{ marginTop: "15px", color: "#64748b", fontWeight: "600" }}>Gemini está analizando las brechas de aprendizaje...</p>
              </div>
            ) : (
              <>
                <p style={{ color: "#1e293b", lineHeight: "1.7", backgroundColor: "#1B396A10", padding: "25px", borderRadius: "18px", borderLeft: "6px solid #1B396A", fontSize: "1.1rem" }}>
