...
const STATUS_COLOR: Record<string, { text: string; bg: string }> = {
  ...
  published: { text: "#1B396A", bg: "#1B396A15" },
...
const ExamHoverCard = ({ exam, courseId, onStatusChange }: { exam: ExamListItem, courseId: string, onStatusChange: (examId: string, status: string) => void }) => {
  ...
  const { changingStatus, handleStatusChange, feedback, setFeedback } = useExamCard(exam, onStatusChange);
  ...
  return (
    ...
    >
      ...
      <div style={{ borderTop: "1px solid #f1f5f9", padding: "14px 18px", display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        {/* Fila 2: Herramientas del Docente (Simular, Configurar, Resultados) */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px" }}>
          ...
          <button
            ...
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "white"; e.currentTarget.style.borderColor = "#cbd5e1"; }}
          >
            <Settings size={14} color="#475569" /> Configurar
          </button>
          <button
            type="button"
            ...
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "white"; e.currentTarget.style.borderColor = "#cbd5e1"; }}
          >
            <FileText size={14} color="#475569" /> Resultados
          </button>
        </div>
      </div>
    </div>
...
function EvaluacionesContent({ courseId, reloadKey, onRetry }: { courseId: string; reloadKey: number; onRetry: () => void }) {
  const router = useRouter();
  const [selectedUnitId, setSelectedUnitId] = useState("");
  const { loading, result, evaluaciones, onStatusChange } = useEvaluacionesContent(courseId, reloadKey);
  ...
  return (
    <div style={{ padding: "40px", maxWidth: "1300px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "32px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 14px", backgroundColor: "#1B396A10", borderRadius: "12px", border: "1px solid #1B396A25", width: "fit-content" }}>
        <span style={{ fontWeight: "800", color: "#1B396A", fontSize: "0.75rem", textTransform: "uppercase" }}>Unidad</span>
        <select value={selectedUnitId} onChange={(e) => setSelectedUnitId(e.target.value)} style={{ padding: "6px 10px", borderRadius: "8px", border: "1px solid #1B396A25", outline: "none", fontSize: "0.85rem", fontWeight: "700", color: "#1B396A", backgroundColor: "white", cursor: "pointer" }}>
...
export default function ListaEvaluacionesPage() {
  ...
  const { reloadKey, onReload } = useEvaluaciones(courseId);

  return (
    <EvaluacionesContent courseId={courseId} reloadKey={reloadKey} onRetry={onReload} />
  );
}
