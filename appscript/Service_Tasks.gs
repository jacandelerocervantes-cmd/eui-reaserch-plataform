...
function obtenerListasTareas() {
  try {
    if (typeof Tasks === 'undefined' || !Tasks.Tasklists) return [];
    const taskLists = Tasks.Tasklists.list();
...
function obtenerTareasPorLista(idLista) {
  try {
    if (typeof Tasks === 'undefined' || !Tasks.Tasks) return [];
...
function crearTareaRapida(payload) {
  try {
    if (typeof Tasks === 'undefined' || !Tasks.Tasklists) throw new Error("Servicio Tasks no disponible");
