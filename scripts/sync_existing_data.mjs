...
for (const [name, value] of Object.entries({ SUPABASE_URL, SERVICE_KEY, APPS_SCRIPT_URL, APPS_SCRIPT_SECRET })) {
  if (!value) {
    throw new Error(`Falta la variable de entorno ${name}. Ejecuta este script con: node --env-file=env.local scripts/sync_existing_data.mjs`);
  }
}
...
async function run() {
  console.log("=== SINCRONIZACI�N COMPLETA SUPABASE -> GOOGLE SHEETS ===");
  ...
  for (const course of courses) {
    ...
    // Sincronizar Pesta�a LISTA_ALUMNOS con Equipos
    ...
    console.log(`  Sesiones �nicas a sincronizar en UNIDAD 1: ${sessionKeys.length}`);
  ...
  console.log("\n======================================================");
  console.log("=== SINCRONIZACI�N COMPLETADA CON TOTAL �XITO ===");
  console.log("======================================================");
}
