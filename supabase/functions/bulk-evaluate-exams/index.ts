...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: exam } = await serviceClient
      .from("exams")
      .select("id, title, unit_id, course_units(course_id)")
      .eq("id", examId)
      .single()
    ...
    const courseId = (exam as any).course_units?.course_id as string | undefined
    ...
    let confidenceThreshold = 0.3
    {
      const { data: courseCalibration } = await serviceClient
        ...
        .eq("domain", "exam_grading").eq("course_id", courseId)
