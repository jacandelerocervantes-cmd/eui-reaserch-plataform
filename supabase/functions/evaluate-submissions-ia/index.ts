...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!assignment.course_id || !(await verifyCourseOwnership(serviceClient, assignment.course_id, userId))) {
      return new Response(
        JSON.stringify({ error: "No tienes permiso sobre esta actividad." }),
