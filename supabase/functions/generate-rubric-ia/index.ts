...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      try { currentRubrics = JSON.parse(String(formData.get("current_rubrics") ?? "[]")) } catch { currentRubrics = [] }
      ...
      currentRubrics = Array.isArray(body.current_rubrics) ? body.current_rubrics : []
    }
...
- Devuelve ÚNICAMENTE un JSON:
{
  "words": [
    ...
    // ── 3. MODO NORMAL: REDACCIÓN DE INSTRUCCIONES + RÚBRICA POR COMPETENCIAS ─
    // Soporta turnos de chat: si `currentRubrics` trae una rúbrica ya trabajada
    // por el docente en esta conversación, AJUSTA en vez de regenerar desde
    // cero — necesario para que "hazla más estricta" o "cambia el criterio 2"
    ...
    const promptText =
      ...
      - Texto dirigido al alumno, en español, claro y específico sobre qué debe entregar y cómo.
      - 3 a 6 oraciones. Sin encabezados ni markdown, texto plano.

      REQUISITOS DE LA RÚBRICA ("rubrics"):
      ...
      JSON puro sin markdown, con AMBAS partes completas (rubrics = todos los criterios finales, no solo los que cambiaron):
      {"instructions":"Texto de instrucciones para el alumno...","rubrics":[{"id":1,"name":"Nombre del Criterio","description":"Indicadores de desempeño observables y medibles para este criterio.","weight":35}]}`
    ...
    try {
      ...
      if (aiRes.ok) {
        ...
        if (content) {
          const parsed = JSON.parse(content)
          // Compatibilidad hacia atrás: si algún día Gemini devuelve el array
          // plano de antes (sin "instructions"), lo seguimos aceptando.
          if (Array.isArray(parsed)) { rubrics = parsed } else {
            rubrics = Array.isArray(parsed.rubrics) ? parsed.rubrics : []
            instructionsText = typeof parsed.instructions === "string" ? parsed.instructions : ""
          }
        }
      }
    } catch (geminiErr) {
    ...
    const guard = await guardOutputOrBlock(JSON.stringify({ instructions: instructionsText, rubrics }), {
    ...
    return new Response(
      JSON.stringify({ success: true, rubrics, instructions: instructionsText || undefined }),
      { headers: { ...cors, "Content-Type": "application/json" } }
    )
