...
// Ver docs/QA_RED_TEAM_DOCENTE_2026-09-14.md — Fase 2. Resuelve en lote el
// student_profile_id de cada matrícula del payload: reusa el existente si ya
// hay una persona con esa matrícula (en cualquier materia), o crea una nueva.

async function resolveOrCreateStudentProfiles(
  serviceClient: SupabaseClient,
  studentsList: StudentInput[],
  matriculas: string[],
): Promise<Map<string, string>> {
  const map = new Map<string, string>();
  if (matriculas.length === 0) return map;

  const { data: existingProfiles } = await serviceClient
    .from("student_profiles").select("id, matricula").in("matricula", matriculas);
  for (const p of (existingProfiles || []) as { id: string; matricula: string }[]) {
    map.set(p.matricula, p.id);
  }

  const faltantes = matriculas.filter((m) => !map.has(m));
  if (faltantes.length === 0) return map;

  const porMatricula = new Map(studentsList.map((s) => [String(s.matricula || "").trim(), s]));
  const nuevosPerfiles = faltantes.map((m) => {
    const est = porMatricula.get(m);
    return {
      matricula: m,
      apellido_paterno: String(est?.apellido_paterno || "DESCONOCIDO").toUpperCase().trim(),
      apellido_materno: est?.apellido_materno ? String(est.apellido_materno).toUpperCase().trim() : null,
      nombres: String(est?.nombres || "DESCONOCIDO").toUpperCase().trim(),
      correo: est?.correo && typeof est.correo === "string" ? est.correo.toLowerCase().trim() : null,
    };
  });

  // ignoreDuplicates + select: si dos matrículas nuevas coinciden con una
  // carrera concurrente (otra importación al mismo tiempo), el conflicto se
  // ignora en vez de tirar la importación completa — se recupera el id real
  // en la relectura de abajo.
  await serviceClient.from("student_profiles").upsert(nuevosPerfiles, { onConflict: "matricula", ignoreDuplicates: true });

  const { data: reread } = await serviceClient
    .from("student_profiles").select("id, matricula").in("matricula", faltantes);
  for (const p of (reread || []) as { id: string; matricula: string }[]) {
    map.set(p.matricula, p.id);
  }
...
async function executeSmartMerge(
  ...
  const matriculasLimpias = unique
    .map((est) => (est.matricula ? String(est.matricula).trim() : null))
    .filter((m): m is string => !!m);
  const profileIdByMatricula = await resolveOrCreateStudentProfiles(serviceClient, unique, matriculasLimpias);
  ...
  for (const est of unique) {
    ...
    const studentProfileId = matriculaIA ? profileIdByMatricula.get(matriculaIA) ?? null : null;
    ...
    if (match) {
      toUpdate.push({
        ...
        student_profile_id: studentProfileId ?? (match.student_profile_id as string | null | undefined) ?? null,
      });
    } else {
      toInsert.push({
        course_id:          courseId,
        ...
        correo:             normalizeEmail(est.correo) || (matriculaIA ? `${matriculaIA}@tizimin.tecnm.mx` : null),
