// supabase/functions/sync-grading-matrix/index.ts
...
serve(async (req: Request) => {
  ...
  try {
    const { courseId, matrixData } = await req.json();
    ...
    if (materia?.google_sheet_id) {
      const response = await fetch(Deno.env.get('APPS_SCRIPT_URL')!, {
        ...
        body: JSON.stringify({ 
          ...
          payload: { googleSheetId: materia.google_sheet_id, dataMatrix: matrixData }
        })
      });
      const result = await response.json();
      return new Response(JSON.stringify(result), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    throw new Error("No hay Sheet vinculado");
  } catch (error: any) {
