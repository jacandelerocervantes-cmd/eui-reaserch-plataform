...
-- Hecho idempotente (2026-09-14): este bloque ya se había aplicado
-- directamente en producción en una sesión anterior sin pasar por el
-- historial de migraciones — confirmado por lectura contra el schema real
-- (creado_en/url_pdf ya no existen, autores ya es text[]). Se envuelve cada
-- paso en un chequeo condicional para que esta migración sea segura de
-- re-ejecutar sin importar en qué estado esté la base.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'literatura_referencias' AND column_name = 'creado_en'
  ) THEN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'literatura_referencias' AND column_name = 'url_pdf'
  ) THEN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'literatura_referencias' AND column_name = 'autores'
      AND data_type <> 'ARRAY'
  ) THEN
...
-- "perfiles" (español) nunca existió como tabla real en este proyecto — la
-- tabla vigente es "profiles" (inglés, confirmado por el usuario 2026-09-14:
-- "todas las tablas en inglés son las correctas"). Su policy de SELECT ya
-- está restringida a authenticated (verificado contra el schema real), así
-- que no hace falta ningún ajuste aquí — se omite el bloque perfiles.

DROP POLICY IF EXISTS "Inserción abierta de sensores" ON public.telemetria_iot;
DROP POLICY IF EXISTS "Inserción de sensores autenticada" ON public.telemetria_iot;
...
DROP POLICY IF EXISTS "Lectura de telemetría autenticada" ON public.telemetria_iot;
