...
6. `006_rls_hardening_kalman_version.sql` — cierra la policy RLS pública de `telemetria_iot` (insert/select sin sesión) y agrega `kalman_states.version`. **Actualizado 2026-09-14**: la parte que cerraba una policy pública en `perfiles` se quitó porque esa tabla ya no existe en producción (reemplazada por `profiles`+`students`); correrla tal cual rompía toda la transacción.
7. ~~`007_rls_equipos_lab.sql`~~ — **eliminado 2026-09-14**. Describía una policy pública de `equipos_lab` que ya no existe en producción (la tabla solo tiene policies restringidas a rol docente); aplicarlo habría agregado una policy nueva abriendo lectura a cualquier autenticado, un retroceso de seguridad. No hacía falta ningún fix ahí.
